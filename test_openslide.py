...
def return_random_patch(whole_slide):
    ...
    cropped_image = whole_slide.read_region((random_location_x, random_location_y), 0, (256, 256))
    while np.sum(np.any(np.array(cropped_image)[:, :, :-1] == [255.0, 255.0, 255.0], axis=-1)) > 0.2 * 256 * 256:
        random_location_x = random.randint(0, wsi_dimensions[0] - 256)
        random_location_y = random.randint(0, wsi_dimensions[1] - 256)
        cropped_image = whole_slide.read_region((random_location_x, random_location_y), 0, (256, 256))
