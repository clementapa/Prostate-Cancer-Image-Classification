...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        padded_im_tensor = F.pad(im_tensor,
            pad=(0, 0, quantity_to_pad*bool_temp, 0, quantity_to_pad*(1 - bool_temp), 0), 
        ...
        if remaining_pixels != 0:
            if (padded_im_tensor.shape[1] + remaining_pixels) % self.params.patch_size == 0:
                # padd
                ...
                padded_im_tensor = padded_im_tensor[:, 0:padded_im_tensor.shape[1] -
                                                    remaining_pixels, 0:padded_im_tensor.shape[2]-remaining_pixels, :]

        h = (padded_im_tensor.shape[1] // self.params.patch_size)
        w = (padded_im_tensor.shape[2] // self.params.patch_size)
        output_patches = rearrange(padded_im_tensor, 'b (h p1) (w p2) c -> b (h w) p1 p2 c',
                                   p1=self.params.patch_size, p2=self.params.patch_size, h=h, w=w)

        mask = (1.0*(output_patches == 255)).sum(dim=(2, 3, 4))/(self.params.patch_size *
