...
def main():
    ...
    artifact_test = wandb_run.use_artifact(
        "attributes_classification_celeba/dlmi/test_256_1_0.5_score:v0", type="dataset"
    )
    ...
    artifact_train = wandb_run.use_artifact(
        "attributes_classification_celeba/dlmi/train_256_1_0.5_score:v0", type="dataset"
    )
    ...
    df_train = pd.read_csv(
        os.path.join(parameters.data_param.root_dataset, "train.csv")
    )
    ...
    X_train = np.array(
        [
            np.mean(np.load(open(os.path.join(path_arrays_train, file), "rb")), axis=0)
            for file in name_train_scores
        ]
    )
    X_test = np.array(
        [
            np.mean(np.load(open(os.path.join(path_arrays_test, file), "rb")), axis=0)
            for file in name_test_scores
        ]
    )
    y_train = [
        df_train[df_train["image_id"] == name.replace("_score.npy", "")][
            "isup_grade"
        ].values[0]
        for name in name_train_scores
    ]
